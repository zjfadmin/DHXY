...
function insertServiceToService(){
	...
	if($("#userName").val()==null)//区域名
		{
		alert("用户不能为空");
		return;
		}
	if($("#userpwd").val()==null)//区域Id
	{
		alert("密码不能为空");
		return;
		}
	if($("#useragentName").val()==null)//分成比例
	{
		alert("代理者姓名不为空");
		return;
		}
	if($("#userQuYuID").val()==null)//代理者
	{
		alert("区域ID不能为空");
		return;
		}
	...
	initService();
	//设置页码
	userManage.controlStyle=10000;
	//区域名
	userManage.username=$("#userName").val();
	//区域ID
	userManage.pwd=$("#userpwd").val();
	//代理者
	userManage.relname=$("#useragentName").val();
	//登录标识
	requestBean.requresHeader='10006';
	requestBean.style="request";
	requestBean.content=JSON.stringify(userManage);
